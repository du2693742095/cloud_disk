...
int main(int argc, char *argv[])
{
    ...
    //初始化服务器参数
    char ip[16] = {0};
    int port = 0, connectSize, threadNum = 0, taskQueueSize = 0;
    configInit(argv[1], ip, &port, &connectSize, &threadNum, &taskQueueSize);
    ...
    //创建线程池
    threadPoll_t *pThreadPoll = (threadPoll_t *)malloc(sizeof(threadPoll_t));
    ...
    while(true){
        //暂时不设置超时
        do{
            nready = epoll_wait(epollfd, pevList, connectSize, -1);
